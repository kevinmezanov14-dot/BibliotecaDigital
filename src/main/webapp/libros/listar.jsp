...
<body>
	...
	<div class="container">
		...
		<c:if test="${param.error == 'id_invalido'}">
			<div class="alert alert-danger">
				❌ Error: El ID del libro es inválido
			</div>
		</c:if>

		<c:if test="${param.error == 'eliminar'}">
			<div class="alert alert-danger">
				❌ Error al eliminar el libro. Intenta nuevamente.
			</div>
		</c:if>
		...
		<div style="margin-bottom: 15px;">
			<a href="<%=request.getContextPath()%>/libros/registrar">
				➕ Agregar Nuevo Libro
			</a>
		</div>
			...
			<tbody>
				<c:forEach var="libro" items="${libros}">
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${libro.stock > 0}">
									<span style="color: green; font-weight: bold;">${libro.stock}</span>
								</c:when>
								<c:otherwise>
						...
						<td>
							<!-- Editar -->
							<a href="<%=request.getContextPath()%>/actualizar/libros?id=${libro.id}">
								✏️ Editar
							</a>
							
							<!-- Eliminar - FORMULARIO POST -->
							<form action="<%=request.getContextPath()%>/eliminar/libros" 
							      method="post" 
